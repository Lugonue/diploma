...
const Header = (props: Props) => {
  ...
  return (
    <header
      className={cn(
        "pt w-full fixed top-0 z-50 ",
        hasScrolled ? "shadow-lg" : ""
      )}
    >
      <div className="flex items-center justify-between bg-gradient-to-t from-gray-200 to-gray-300 py-5 px-10 rounded ">
        <div className="flex items-center gap-2">
          <img width={30} src="/img/logo.webp" alt="" />
          <span
            className="text-primary-100 cursor-pointer font-bold text-2xl"
            onClick={() => navigate("/")}
          >
            {" "}
            {t("title")}
        ...
        <div
          ref={menuRef}
          className={cn(
            `flex-wrap gap-2 hidden md:flex ${open ? "flex flex-col items-start absolute top-10 right-10 bg-white p-5 rounded" : ""}`
          )}
        >
